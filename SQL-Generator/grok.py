'''
DESCR: THIS IS MAIN FILE OF CHAT BOT. THIS FILE CONTAINS LLM MODELS AND MONGO DB.

        IF USER INPUT IS DATA-RELATED LLM MODEL WILL GENERATE MONGO_DB QUERY. 
        THEN APPLICATION EXECUTE THAT QUERY AND FLASH TO USER. OTHER WISE APPLICATION WILL RESPOND 
        BASED ON USER QUERY.
...
prompt = [
    {
        ...
        "content": f"""
            ...
            - Always return your answer strictly as a single line of valid Python code in the format:

            -Mongo_DB_FindQuery = [collection.<your_query_here>]

            Rules:
            - If the query does not have square brackets, wrap it inside [ ] automatically.
            - Replace <your_query_here> with the extracted MongoDB query only (everything after "collection.").
            - Do not add explanations, comments, or extra text.
            - The output must be exactly one Python assignment statement.                  
...
if user_input := st.chat_input("Enter your query"):
    ...
    if match:
        ...
        with st.chat_message("assistant"):
            ...
            if formatted_output:
                st.dataframe(formatted_output, width=800, height=300)
